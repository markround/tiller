...
### XML File plugin
This plugin parses the contents of an XML file (for example, a Maven POM) and makes the resulting structure available to your templates. You can enable this plugin by adding `xml_file` to your list of datasources in `common.yaml`. One this has been done, you can specify two further parameters; either at the top-level of `common.yaml`, or per-template. These parameters are :
